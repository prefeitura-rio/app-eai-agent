...
def test_system_prompt_injection():
    ...
    try:
        ...
        technical_indicators = ["lambda", "função", "def", "return", "parâmetro", "argumento"]
        is_technical = any(indicator in response.message.lower() for indicator in technical_indicators)
        ...
        is_technical_2 = any(indicator in response4.message.lower() for indicator in technical_indicators)
