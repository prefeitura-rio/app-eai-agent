...
export default function ChatClient() {
  ...
  const handleDeleteHistory = async () => {
    ...
    try {
      ...
      const totalDeleted =
        (deleteResult.tables.checkpoints.deleted_rows || 0) +
        (deleteResult.tables.checkpoints_writes.deleted_rows || 0);

      if (deleteResult.overall_result === "success") {
        toast.success(
          `Histórico deletado com sucesso! ${totalDeleted} registros removidos.`
        );
      } else {
        toast.warning(
          `Operação parcial: ${totalDeleted} registros removidos. Verifique os detalhes.`
        );
      }
    } catch (error) {
      const errorMessage =
