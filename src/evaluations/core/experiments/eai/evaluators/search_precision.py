...
class SearchPrecisionEvaluator(BaseOneTurnEvaluator):
    """
    Avalia a precisão dos documentos retornados pelo google_search comparando
    com uma lista de documentos esperados (golden_documents_list).
    
    Precisão = (número de documentos golden retornados) / (total de documentos retornados)
    ...
    Se há múltiplas chamadas ao google_search, retorna a precisão máxima.
    """
    ...
    async def evaluate(
        ...
        try:
            ...
            # Calcula a precisão para cada chamada e pega a máxima
            precisions = []
            precision_details = []
            ...
            for call_idx, returned_ids in enumerate(search_calls):
                if not returned_ids:
                    ...
                    matches = [doc_id for doc_id in returned_ids if doc_id in golden_docs]
                    precision = len(matches) / len(returned_ids)
            ...
            max_precision_idx = precisions.index(max_precision)
            ...
            return EvaluationResult(
                ...
                annotations={
                    ...
                    "max_precision_call_index": max_precision_idx,
