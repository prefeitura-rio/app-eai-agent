...
def eval_method(name: str, turns: str) -> Callable:
    ...
    def decorator(func: Callable[..., Awaitable[EvaluationResult]]) -> Callable:
...
class Evals:
    ...
    async def _get_llm_judgement(
        ...
        """
        Formata um prompt, executa-o contra o cliente juiz e retorna um resultado padronizado.
        """
        try:
            task_dict = task.model_dump(exclude_none=True)

            # Para prompts que usam chaves como {task[persona]}, o dicionário é necessário.
            # Para respostas de agente, pode ser um objeto Pydantic ou um dicionário (no caso de multi-turno).
            response_dict = (
                agent_response
            ...
            logger.error(f"Erro durante o julgamento do LLM: {e}", exc_info=True)
