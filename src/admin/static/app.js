...
document.addEventListener("DOMContentLoaded", function () {
  ("DOM carregado, inicializando painel admin");
  ...
  document.addEventListener("click", function (e) {
    ...
    if (
      e.target.matches(
        '.prompt-editor, #memoryBlocks, #tools, input[type="text"], textarea'
      ) ||
...
function handleLogin(e) {
  ...
  fetch(
    `${API_BASE_URL}/api/v1/system-prompt/history?agent_type=agentic_search`,
    {
...
function handleSaveAll() {
  ...
  confirmSaveBtn.addEventListener("click", function () {
    ...
    if (!authorModalInput.value.trim()) {
      document
        .getElementById("authorUnifiedModalFeedback")
    ...
    if (!reasonModalInput.value.trim()) {
      document
        .getElementById("reasonUnifiedModalFeedback")
        .classList.remove("d-none");
      return;
    } else {
      document
...
function proceedWithUnifiedSave(
  agentType,
  newPrompt,
  memoryBlocksArray,
  author,
...
function loadData() {
  ...
  Promise.all([
    ...
    apiRequest(
      "GET",
      `${API_BASE_URL}/api/v1/system-prompt?agent_type=${agentType}`
    ).catch((error) => {
      if (
        error.message &&
        error.message.includes("Nenhum system prompt encontrado")
      ) {
    ...
    apiRequest(
      "GET",
      `${API_BASE_URL}/api/v1/agent-config?agent_type=${agentType}`
    ).catch((err) => ({
      memory_blocks: "",
      tools: [],
      model_name: "",
    ...
    apiRequest(
      "GET",
      `${API_BASE_URL}/api/v1/unified-history?agent_type=${agentType}&limit=50`
    ).catch((error) => {
    ...
    .then(
      ([currentPromptData, currentConfigData, unifiedHistoryResponse]) => {
        ...
        memoryBlocksText.value = JSON.stringify(
          currentConfigData.memory_blocks || [],
          null,
          2
        );
...
function renderBackendUnifiedHistory(historyItems) {
  ...
  if (historyItems.length === 0) {
    historyList.innerHTML =
      '<div class="text-muted p-3">Nenhum histórico disponível</div>';
    return;
  }
  ...
  historyItems.forEach((item) => {
    ...
    const versionDisplay =
      ...
      `eai-${new Date(item.created_at).toISOString().split("T")[0]}-v${
        item.version_number
      }`;
  ...
  fetch(url, {
    ...
    .catch((error) => {
      ...
      if (localPrompt && localPrompt.content) {
        ...
        updateActiveHistoryItem(promptId);
        showAlert(
          "Usando dados locais do prompt (podem estar incompletos)",
          "warning"
        );
      } else {
        showAlert(
          "Erro ao carregar dados do prompt: " + error.message,
          "danger"
        );
      }
  ...
  apiRequest(
    ...
    .then((versionDetails) => {
      ...
      if (versionDetails.config) {
        memoryBlocksText.value = JSON.stringify(
          versionDetails.config.memory_blocks || [],
          null,
          2
        );
...
function updateActiveBackendHistoryItem(selectedItem) {
  ...
  document.querySelectorAll(".history-item").forEach((item) => {
    if (
      item.dataset.versionNumber === selectedItem.version_number.toString()
    ) {
...
function handleResetAll() {
  ...
  if (
    !confirm(
      "Tem certeza que deseja resetar COMPLETAMENTE o banco de dados para este tipo de agente? Esta ação removerá TODO o histórico e recriará versões padrão. Esta ação NÃO pode ser desfeita."
    )
  ) {
    return;
  }
  ...
  apiRequest(
    ...
    .then((response) => {
      ...
      let message =
        response.message || "Reset unificado concluído com sucesso!";
    ...
    .catch((error) => {
      ...
      showAlert(
        "Erro ao executar reset unificado: " +
          (error.message || "Erro desconhecido"),
        "danger"
      );
    });
